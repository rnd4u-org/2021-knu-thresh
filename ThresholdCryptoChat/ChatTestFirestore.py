...
firebase_admin.initialize_app(cred)
db = firestore.client()
#vusers = [u'karina', u'dmytro', u'pavlo']
...
def on_snapshot(collection_snapshot, changes, read_time):
    for doc in collection_snapshot:
        if doc.id not in messages and doc.to_dict()[u'user'] != 'pasha':
            messages.append(doc.id)
            print(u"{} send message: {}".format(doc.to_dict()[u'user'], doc.to_dict()[u'message'].decode('utf-8')))
    callback_done.set()
...
while shutdown == False:
    try:
        ...
        if message != "":
            ...
            doc_ref.set({u'message': message, u'user': u'pasha'})
