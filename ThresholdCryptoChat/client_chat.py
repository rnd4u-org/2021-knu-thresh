import json, time
from threshold_crypto import ThresholdParameters, KeyParameters, PublicKey, KeyShare, ThresholdCrypto, EncryptedMessage, PartialDecryption
...
def clear_collection(db):
    ...
    for d in data.stream():
        d.reference.delete()
    data = db.collection(u'crypto_message')
    for d in data.stream():
        d.reference.delete()
